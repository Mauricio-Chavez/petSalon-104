...
function displayNames(){
    ...
    if (reverse) {
        for (let i = pets.length - 1; i >= 0; i--) {
            text += `<tr><td>${pets[i].name}</td><td>${pets[i].age}</td><td  class ='none-display'>${pets[i].gender}</td><td>${pets[i].owner}</td><td class ='none-display'>${pets[i].species}</td><td><button class="delete-btn" onClick="deleteElement('${pets[i].name}')">Delete</button></td></tr>`;
        }
    } else {
...
function displayPetCards(){
    ...
    for (let i = 0; i < salon.pets.length; i++) {
        ...
        card += `
            <div class="petCard" style="background-color: #${bColor}; color: #ffffff;">
                <div class="detele-container"><button class="delete-btn" onClick="deleteElement('${pet.name}')">X</button></div> 
                <div>
